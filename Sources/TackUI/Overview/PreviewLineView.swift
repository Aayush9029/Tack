    ...
    var body: some View {
        ...
        case let .bullet(text):
            Label { Text(text).lineLimit(2) } icon: { Text("•").foregroundStyle(.secondary) }
                .font(.system(size: 14))
        case let .quote(text):
            Text(text).font(.system(size: 14)).foregroundStyle(.secondary).lineLimit(2)
                .padding(.leading, 8)
                .overlay(alignment: .leading) { Capsule().fill(.tertiary).frame(width: 2) }
        case let .code(text):
            Text(text).font(.system(size: 13, design: .monospaced)).foregroundStyle(.secondary).lineLimit(1)
        case let .text(text):
            Text(text).font(.system(size: 14)).lineLimit(3)
        }
    }
}
